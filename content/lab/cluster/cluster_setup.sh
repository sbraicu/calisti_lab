...
function create_cluster {
    ...
    if [[ "$CLUSTER_TYPE" == "k3d" ]]; then
        ...
        k3d cluster create ${name} --no-lb --image 'rancher/k3s:v1.23.8-k3s2' --api-port ${apiaddr}:${apiport} --network k3d-${K3D_NAMEPRFX} --k3s-arg '--disable=servicelb' --k3s-arg '--disable=traefik' ${K3D_API_SAN:- --k3s-arg "--tls-san=${K3D_API_SAN}"} --k3s-arg '--disable-network-policy' --k3s-arg '--flannel-backend=none' --k3s-arg 'cloud-provider=external' --k3s-arg '--disable-cloud-controller' --volume "$(pwd)/calico.yaml:/var/lib/rancher/k3s/server/manifests/calico.yaml" --volume $(pwd)/resolv.conf:/etc/resolv.conf
